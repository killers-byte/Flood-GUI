-- ============================================
-- TROXZY VIP v24.18 "SPECTRAL BLADE" [BACKEND CORE - COORDINATE SPECTATOR]
-- Badan Intelijen Negara - HYBRID SYSTEM + LOBBY/LIFT PLAYER TRACKER
-- 🔥 HANYA MUNCUL SAAT USER DI MAP, BERDASARKAN KOORDINAT
-- ============================================
...
local function supremeKeyValidation()
    ...
    local function checkKey(rawInput)
        ...
        wait(0.5); keyValid = true; getgenv().keyExpireTime = expireTime; KeyScreen:Destroy()
    end
    ...
    repeat wait() until keyValid or not Player.Parent
    return keyValid
end
...
local function playSound(id) pcall(function() local s = getPooledSound(); s.SoundId = "rbxassetid://" .. id; s.Parent = Workspace; s:Play(); spawn(function() wait(3); returnSoundToPool(s) end) end) end
...
-- 🔥 AKURAT ADMIN DETECTION
local function isAdminName(str)
    str = str:lower()
    local patterns = {
        "^[@_]?[aà]dm?[i1]n[istrator]*$", "^m[o0]d[erato]*r?$", "^st[a4]ff$", "^[o0]wn[e3]r$",
        "^c[o0][-]?[o0]wn[e3]r$", "^h[e3]ad[aà]dm?[i1]n$", "^h[e3]adm[o0]d$", "^d[e3]v[e3]l[o0]p[e3]r$",
        "^d[e3]v$", "^h[e3]lp[e3]r$", "^m[aà]n[aà]g[e3]r$", "^f[o0]und[e3]r$", "^s[e3]ni[o0]r[aà]dm?[i1]n$",
        "^j[u]r[i]u$", "^p[e3]ngurus$", "^p[e3]nj[aà]g[aà]$", "^k[e3]p[aà]l[aà]$", "^p[e3]m[i]mp[i]n$", "^t[e3]st[e3]r$"
    }
    for _, pattern in ipairs(patterns) do if str:match(pattern) then return true end end
    local keywords = { "admin", "mod", "owner", "dev", "helper", "staff", "manager", "founder", "head", "leader", "pengurus", "penjaga", "kepala", "pemimpin", "pengawas", "tester" }
    for _, kw in ipairs(keywords) do if str:find(kw, 1, true) then return true end end
    return false
end

local function isAdmin(p)
    if isAdminName(p.Name) or isAdminName(p.DisplayName) then return true end
    ...
    if leaderstats then
        for _, stat in ipairs(leaderstats:GetChildren()) do
            if stat:IsA("StringValue") or stat:IsA("IntValue") or stat:IsA("NumberValue") then
                ...
                if lowerName == "admin" or lowerName == "mod" or lowerName == "rank" then
                    local val = tostring(stat.Value):lower()
                    if val == "admin" or val == "mod" or val == "owner" or val == "dev" then return true end
                end
            end
        end
    end
...
-- ==============================================
-- 👁️ SPECTATOR: PEMAIN DI LOBBY/LIFT (BASIS KOORDINAT)
-- HANYA MUNCUL SAAT USER DI DALAM MAP (X > 50)
-- ==============================================
local function getPlayersInLobbyLift()
    ...
    if not myHrp or myHrp.Position.X <= 50 then
        -- User tidak di dalam map, kembalikan kosong
        return entries
    end
    
    -- User di dalam map, lacak pemain lain di lobby/lift
...
local function getSpectatorDisplayNames()
    ...
    for _, entry in ipairs(entries) do
        table.insert(displayNames, entry.name .. " [ " .. entry.location .. " ]")
    end
    return displayNames
end
...
local function getSpectators()
    ...
    for _, p in ipairs(Players:GetPlayers()) do
        if p ~= Player and p.Character then
            ...
            if hrp and hrp.Position.X < 50 then
...
local function getSpectatorNames()
    return getSpectatorDisplayNames()
end

local function activateCounterMeasures()
    if STATE.CounterMeasureActive then return end
    STATE.CounterMeasureActive = true
end
local function deactivateCounterMeasures()
    if not STATE.CounterMeasureActive then return end
    STATE.CounterMeasureActive = false
end
...
-- 🔧 SPECTATOR UI UPDATE (INDEPENDEN DARI ADMIN DETECTOR)
local function updateSpectatorUI()
    ...
    if #specNames > 0 then
        if not STATE.CounterMeasureActive then activateCounterMeasures() end
    else
        if STATE.CounterMeasureActive then deactivateCounterMeasures() end
    end
end
...
local function updateAdminDetection()
    ...
    if adminCount > 0 then
        if adminCount > lastAdminCount and (os.clock() - lastAdminAlert >= 10) then
            ...
            if not STATE.isGhostMode then activateGhostMode() end
        end
    else
...
local function forceReconnect() saveStats(); wait(1); pcall(function() TeleportService:TeleportToPlaceInstance(game.PlaceId, game.JobId, Player) end); wait(2); pcall(function() TeleportService:Teleport(game.PlaceId) end) end
local function attemptReconnect() saveStats(); wait(3); pcall(function() TeleportService:TeleportToPlaceInstance(game.PlaceId, game.JobId, Player) end) end
...
local function ExecuteTAS()
    ...
    if STATE.TAS_RUNNING then TAS_COROUTINE = nil; STATE.TAS_RUNNING = false; wait(0.2) end
...
local function updateESP()
    ...
    for _, plr in ipairs(allPlayers) do
        if plr ~= Player and plr.Character and not espCache[plr] then
            ...
            hl.Name = "TroxzyESP"; hl.FillTransparency = 0.3; hl.OutlineTransparency = 0.1; hl.Parent = plr.Character
            espCache[plr] = hl
        end
...
function StartWinEngine()
    ...
    if not func then WinDownloading = false; getgenv().TomatoAutoFarm = false; STATE.CurrentlyFarming = false; notify("Kesalahan kompilasi script Auto Farm!", "Error"); return end
...
-- AUTO UPDATER SYSTEM v1.1 (Lua 5.1 safe)
local UPDATE_CONFIG = {
    ENABLED = true, 
...
local function performUpdate()
    notify("🔄 Update ditemukan! Membersihkan total script lama...", "Auto Updater")
    ...
    local function purgeTroxzyGuis(container) for _, obj in ipairs(container:GetChildren()) do if obj:IsA("ScreenGui") and string.find(obj.Name:lower(), "troxzy") then pcall(function() obj:Destroy() end) end end end
    pcall(function() purgeTroxzyGuis(game:GetService("CoreGui")) end); pcall(function() purgeTroxzyGuis(game:GetService("Players").LocalPlayer:WaitForChild("PlayerGui")) end)
    ...
    if scriptData then notify("⚡ Memasang & merestart script baru...", "Auto Updater"); wait(0.8); local func, err = loadstring(scriptData); if func then getgenv().TroxzyAPI = nil; func() else notify("❌ Gagal memuat compile: " .. tostring(err), "Auto Updater") end else notify("❌ Gagal mengunduh update.", "Auto Updater") end
end
local function checkForUpdates() local latestVersion = fetchVersion(UPDATE_CONFIG.VERSION_CHECK_URL); if latestVersion then if latestVersion ~= UPDATE_CONFIG.CURRENT_VERSION then notify("🆕 Update tersedia! " .. UPDATE_CONFIG.CURRENT_VERSION .. " → " .. latestVersion, "Auto Updater"); wait(0.5); performUpdate() else notify("✅ Versi terbaru digunakan (" .. UPDATE_CONFIG.CURRENT_VERSION .. ").", "Auto Updater") end else performUpdate() end end
if CONFIG.AUTO_UPDATE then spawn(function() wait(3); checkForUpdates() end) end

-- HYBRID AUTO-QUEUE (FIXED)
function StartAutoQueue()
    ...
    AutoQueueListener = Multiplayer.ChildAdded:Connect(function(newMap)
        ...
        local s = newMap:WaitForChild("Settings", 3)
        if not s then STATE.HybridActive = false; return end 
        local mapName = "NewMap"; local retryCount = 0
        while (mapName == "NewMap" or mapName == "Map" or mapName == "") and retryCount < 25 do
            local attrName = s:GetAttribute("MapName"); if not attrName then local val = s:FindFirstChild("MapName"); if val and val:IsA("StringValue") then attrName = val.Value end end
            mapName = attrName or newMap.Name; if mapName == "NewMap" or mapName == "Map" or mapName == "" then wait(0.2); retryCount = retryCount + 1 end
        end
        Stats.currentMap = mapName
        local timeout = os.clock() + 5 
        repeat wait() until Check("InGame") or not STATE.AUTO_QUEUE_ENABLED or os.clock() > timeout
        if not STATE.AUTO_QUEUE_ENABLED then STATE.HybridActive = false; return end
        STATE.mapCompleted = false
        if not Check("InGame") then
            notify("Timeout 5s (" .. mapName .. "), gagal load. Auto Farm Aktif.", "Hybrid")
        ...
        if isMapInTAS(mapName) then
            notify("TAS Support (" .. mapName .. ")! Menjalankan TAS...", "Hybrid System")
            getgenv().TomatoAutoFarm = false; STATE.CurrentlyFarming = false
            StopWinEngine(); spawn(ExecuteTAS)
        else
            notify("TAS Tidak Support (" .. mapName .. ")! Auto Farm Aktif...", "Hybrid System")
            TAS_COROUTINE = nil; STATE.TAS_RUNNING = false
            getgenv().TomatoAutoFarm = true; STATE.CurrentlyFarming = true
            StopWinEngine(); StartWinEngine()
        end
...
-- AUTO REBIRTH SYSTEM v2.0 (Lua 5.1 safe)
...
local function executeRebirth()
    ...
    if RebirthState.foundRemote then 
        ...
        if success then RebirthState.rebirthCount = RebirthState.rebirthCount + 1; RebirthState.lastRebirthTime = os.clock(); notify("🔄 Rebirth Dieksekusi Secara Otomatis!", "Auto Rebirth"); wait(math.random(REBIRTH_CONFIG.REBIRTH_DELAY_MIN, REBIRTH_CONFIG.REBIRTH_DELAY_MAX)); RebirthState.isRebirthing = false; return true end 
    end
    ...
    if RebirthState.foundButton then 
        ...
        if success then RebirthState.rebirthCount = RebirthState.rebirthCount + 1; RebirthState.lastRebirthTime = os.clock(); notify("🔄 Rebirth via Tombol Dieksekusi!", "Auto Rebirth"); wait(math.random(REBIRTH_CONFIG.REBIRTH_DELAY_MIN, REBIRTH_CONFIG.REBIRTH_DELAY_MAX)); RebirthState.isRebirthing = false; return true end 
    end
...
function StartAutoRebirth()
    ...
    spawn(function()
        while REBIRTH_CONFIG.ENABLED and AutoRebirthLoop do
            wait(REBIRTH_CONFIG.CHECK_INTERVAL)
            ...
            if not STATE.panicActive and not RebirthState.isRebirthing then
                ...
                if currentLevel >= REBIRTH_CONFIG.MAX_LEVEL and currentLevel > 0 then
                    ...
                    if wasTAS then TAS_COROUTINE = nil; STATE.TAS_RUNNING = false end
                    wait(0.5); local rebirthSuccess = executeRebirth()
                    if rebirthSuccess and REBIRTH_CONFIG.AUTO_RESUME then
                        ...
                        elseif wasTAS then spawn(ExecuteTAS) end
                    end
                end
            end
        end
...
TrackConnection(RunService.Heartbeat:Connect(function()
    ...
    -- 🔥 SPECTATOR SELALU UPDATE, ADMIN DETECTION TERPISAH
    updateSpectatorUI()
...
notify("TROXZY VIP - LOBBY/LIFT TRACKER AKTIF", "System")
