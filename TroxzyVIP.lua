...
-- 🔄 AUTO-UPDATER & FULL NOTIFICATIONS
-- ============================================
...
local function activatePanicMode() panicActive = true; _G.TroxzyAutoFarm = false; CurrentlyFarming = false; DisconnectMapDetection(); applyNoclip(false); pcall(function() Player.Character.Humanoid.WalkSpeed = 16 end); if Main then Tween(Main, {Size = UDim2.new(0,0,0,0)}, 0.3); task.wait(0.3); Main.Visible = false end; clearESPCache(); notify("Panic Mode has been ACTIVATED!", "Emergency"); if _G.ToggleStates["PANIC_MODE"] then _G.ToggleStates["PANIC_MODE"].SetState(true) end end
local function deactivatePanicMode() panicActive = false; _G.TroxzyAutoFarm = false; CurrentlyFarming = false; pcall(function() Player.Character.Humanoid.WalkSpeed = 16 end); applyNoclip(false); if Main then Main.Visible = true; Tween(Main, {Size = UDim2.new(0,390,0,540)}, 0.4) end; notify("Panic Mode is now DEACTIVATED!", "Emergency"); if _G.ToggleStates["PANIC_MODE"] then _G.ToggleStates["PANIC_MODE"].SetState(false) end end
...
local function AddToggle(tabKey, name, stateKey)
    ...
    btn.MouseButton1Click:Connect(function()
        ...
        elseif stateKey == "AutoFarm" then 
            ...
            notify(name .. " is now " .. statusText, tabKey)
        elseif stateKey == "NIGHT_MODE" then 
            applyTheme(state and "Light" or "Dark")
            notify("Theme set to " .. (state and "Light Theme" or "Dark Theme"), tabKey)
        elseif stateKey == "DASHBOARD" then 
            CONFIG.DASHBOARD = state; if _G.DashboardUI then _G.DashboardUI.Visible = state end
            notify(name .. " is now " .. statusText, tabKey)
...
local function AddInput(tabKey, label, defaultVal, callback)
    ...
    inp.FocusLost:Connect(function() 
        ...
        if v then 
            callback(v)
            notify(label .. " set to " .. tostring(v), tabKey)
        else 
...
AddButton("TAS", "Record Route", DARK_THEME.ButtonRecord, function() 
    if not CONFIG.TAS_AUTO_START then notify("Enable Auto-Start first!"); return end; 
    CONFIG.TAS_MODE="Record"; 
    notify("Starting Recording Route...", "TAS")
    task.spawn(ExecuteTAS) 
end)
AddButton("TAS", "Force Play Route", DARK_THEME.Accent, function() 
    if not CONFIG.TAS_AUTO_START then notify("Enable Auto-Start first!"); return end; 
    CONFIG.TAS_MODE="Play"; 
    notify("Playing TAS Route...", "TAS")
    task.spawn(ExecuteTAS) 
end)
...
AddButton("Stealth", "Force Reconnect", DARK_THEME.ButtonForceLeave, function() notify("Reconnecting to Server...", "Security"); forceReconnect() end)
...
AddButton("Extra", "Cycle Color", DARK_THEME.Accent, function() 
    ...
    local idx=table.find(c,CONFIG.FLOOD_COLOR); idx=idx and (idx%#c)+1 or 1; 
    CONFIG.FLOOD_COLOR=c[idx]; 
    applyFloodColors(); 
    FCLabel.Text="Current: "..CONFIG.FLOOD_COLOR 
    notify("Flood color is now " .. CONFIG.FLOOD_COLOR, "Extra")
end)
