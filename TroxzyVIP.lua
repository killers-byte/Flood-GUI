...
-- 🔥 FEATURE: HYBRID TAS FALLBACK + AUTO REBIRTH V2
-- ============================================
...
local STATE = { AUTO_QUEUE_ENABLED = false, TAS_RUNNING = false, panicActive = false, isGhostMode = false, moveToLift = false, mapCompleted = false, CurrentlyFarming = false }
...
function StartAutoQueue()
    ...
    AutoQueueListener = Multiplayer.ChildAdded:Connect(function(newMap)
        ...
        pcall(function() 
            local s = newMap:WaitForChild("Settings", 5)
            if s then mapName = s:GetAttribute("MapName") or newMap.Name end 
        end)
        Stats.currentMap = mapName

        repeat task.wait() until Check("InGame") or not STATE.AUTO_QUEUE_ENABLED
        if not STATE.AUTO_QUEUE_ENABLED then return end
...
-- =============================================
-- 🔥 AUTO REBIRTH SYSTEM v2.0 (FIXED)
-- =============================================
local REBIRTH_CONFIG = {
    ...
    REMOTE_NAMES = { "rebirth", "reborn", "prestige", "ascend", "reset", "newlife", "dorebirth" },
    BUTTON_NAMES = { "rebirth", "reborn", "prestige", "ascend", "reincarnate" },
    ...
    for _, remote in pairs(ReplicatedStorage:GetDescendants()) do 
        if remote:IsA("RemoteEvent") or remote:IsA("RemoteFunction") or remote:IsA("BindableEvent") or remote:IsA("BindableFunction") then 
            ...
            for _, name in ipairs(REBIRTH_CONFIG.REMOTE_NAMES) do 
                if lowerName:find(name) then return remote end 
            end 
        end 
    end 
    return nil
    ...
    for _, obj in pairs(Workspace:GetDescendants()) do 
        if obj:IsA("ClickDetector") or obj:IsA("ProximityPrompt") then 
            ...
            for _, name in ipairs(REBIRTH_CONFIG.BUTTON_NAMES) do 
                if lowerName:find(name) then return obj end 
            end 
        elseif obj:IsA("BasePart") then 
            ...
            if clickDetector then 
                local lowerName = obj.Name:lower()
                for _, name in ipairs(REBIRTH_CONFIG.BUTTON_NAMES) do 
                    if lowerName:find(name) then return clickDetector end 
                end 
            end 
        end 
    end 
...
local function getCurrentLevel()
    ...
    if leaderstats then 
        ...
        if levelStat then
            if levelStat:IsA("IntValue") or levelStat:IsA("NumberValue") then 
                return levelStat.Value 
            elseif levelStat:IsA("StringValue") then
                local cleanStr = string.gsub(levelStat.Value, "[,KMB]", "")
    ...
    if char then 
        ...
        if type(levelAttr) == "number" then return levelAttr end 
    end
    
    local levelAttr2 = Player:GetAttribute(REBIRTH_CONFIG.LEVEL_ATTRIBUTE); 
    if type(levelAttr2) == "number" then return levelAttr2 end; 
...
local function executeRebirth()
    ...
    if RebirthState.foundRemote then 
        ...
        if success then 
            ...
            notify("🔄 Rebirth Dieksekusi Secara Otomatis!", "Auto Rebirth")
            task.wait(math.random(REBIRTH_CONFIG.REBIRTH_DELAY_MIN, REBIRTH_CONFIG.REBIRTH_DELAY_MAX))
            RebirthState.isRebirthing = false; return true 
        end 
    end
    ...
    if RebirthState.foundButton then 
        local success = pcall(function() 
            if RebirthState.foundButton:IsA("ClickDetector") then fireclickdetector(RebirthState.foundButton) 
            elseif RebirthState.foundButton:IsA("ProximityPrompt") then fireproximityprompt(RebirthState.foundButton) end 
        end)
        if success then 
            RebirthState.rebirthCount = RebirthState.rebirthCount + 1; 
            RebirthState.lastRebirthTime = os.clock(); 
            notify("🔄 Rebirth via Tombol Dieksekusi!", "Auto Rebirth")
            task.wait(math.random(REBIRTH_CONFIG.REBIRTH_DELAY_MIN, REBIRTH_CONFIG.REBIRTH_DELAY_MAX))
            RebirthState.isRebirthing = false; return true 
        end 
    end
    ...
    if RebirthState.attemptCount >= REBIRTH_CONFIG.MAX_RETRIES then 
        notify("⚠️ Gagal mencari trigger Rebirth di game ini.", "Auto Rebirth")
...
-- =============================================
local API = {
    CONFIG = CONFIG, STATE = STATE, Stats = Stats, KeyTime = keyExpireTime, GetRealTime = GetRealTime, getAdminPlayers = getAdminPlayers, getSpectators = getSpectators,
...
TrackConnection(RunService.Heartbeat:Connect(function()
    ...
    pcall(function()
        ...
        if CONFIG.AIR_SWIM then 
            ...
            if hum:GetState() == Enum.HumanoidStateType.Swimming then hum:ChangeState(Enum.HumanoidStateType.GettingUp) end
        else 
...
notify("TROXZY VIP - HYBRID FALLBACK ENABLED", "System")
