...
-- Badan Intelijen Negara - STRICT ADMIN DETECT + HYBRID FIX + AUTO REBIRTH + BRUTAL SCRAPER + NEURAL AI
-- ============================================
...
local CONFIG = {
    ...
    AUTO_REBIRTH = false,
    NEURAL_PATHFINDER = false -- 🧠 FITUR AI BARU
}
...
-- ==========================================
-- 🧠 NEURAL PATHFINDER ENGINE (OBSERVER MODE)
-- ==========================================
_G.NeuralData = _G.NeuralData or {}
local observing = false
local observerConnection = nil
local currentRunData = {}

local function getProPlayer()
    local bestPlayer = nil
    for _, p in ipairs(Players:GetPlayers()) do
        if p ~= Player and p.Character and p.Character:FindFirstChild("HumanoidRootPart") then
            -- Fallback brutal nyari dari character/backpack or random
            bestPlayer = p
            break
        end
    end
...
local function StartObserverMode(mapName)
    if not CONFIG.NEURAL_PATHFINDER or observing then return end
    local target = getProPlayer()
    if not target then return end
    
    _G.NeuralData[mapName] = _G.NeuralData[mapName] or {}
    currentRunData = {}
    local startTime = os.clock()
    observing = true
    
    notify("👁️ AI Mengintai: " .. target.Name .. " di " .. mapName, "Neural Pathfinder")
    observerConnection = RunService.Heartbeat:Connect(function()
        if not target or not target.Character or not target.Character:FindFirstChild("HumanoidRootPart") then 
            StopObserver(false, mapName) 
            return 
        end
        local hrp = target.Character.HumanoidRootPart
        local hum = target.Character:FindFirstChild("Humanoid")
        if hum and hum.Health > 0 then
            table.insert(currentRunData, { Time = os.clock() - startTime, CFrame = hrp.CFrame, Velocity = hrp.Velocity })
        else
            StopObserver(false, mapName)
        end
    end)
...
function StopObserver(success, mapName)
    if observerConnection then observerConnection:Disconnect(); observerConnection = nil end
    observing = false
    if success and #currentRunData > 50 then
        table.insert(_G.NeuralData[mapName], currentRunData)
        notify("🧠 AI Selesai Belajar: " .. mapName .. " (" .. #currentRunData .. " nodes).", "Neural Pathfinder")
    end
end
-- ==========================================
...
function StartWinEngine()
    ...
    WinFarmCoroutine = coroutine.create(function() pcall(func); WinFarmRunning = false; STATE.CurrentlyFarming = false; WinFarmCoroutine = nil end)
                ...
                if gui.Visible then
                    pcall(function()
                        if getconnections then
                            for _, conn in pairs(getconnections(gui.MouseButton1Click)) do conn:Function() end
                        end
                    end)
                    task.wait(0.5) 
                        ...
                        if conf:IsA("TextButton") or conf:IsA("ImageButton") then
                            ...
                            if confText:find("confirm") or confText:find("konfirmasi") or confText:find("yes") or confText:find("ya") then
                                if conf.Visible then
                                    pcall(function()
                                        if getconnections then
                                            for _, conn in pairs(getconnections(conf.MouseButton1Click)) do conn:Function() end
                                        end
                                    end)
...
task.spawn(function() while task.wait(2) do if CONFIG.AUTO_REBIRTH and not STATE.panicActive and not STATE.isGhostMode and not Check("InGame") then pcall(executeAutoRebirth) end end end)

-- ==========================================
-- 💥 BRUTAL GUI SCRAPER (ANTI-HIDDEN STATS)
-- ==========================================
...
local function ScrapeFE2Data()
    ...
    for _, obj in ipairs(pGui:GetDescendants()) do
        if obj:IsA("TextLabel") and obj.Visible then
            ...
            if txt:match("%d+/%d+ XP") or txt:match("%d+ / %d+ XP") then tempStats.XP = txt:gsub(" XP", "") end
            if txt:match("^x%d+$") then tempStats.Rebirths = txt:gsub("x", "") end
            
            local lvlMatch = txt:match("Level%s*(%d+)") or txt:match("Lv%.?%s*(%d+)") or txt:match("Lvl%.?%s*(%d+)")
            if lvlMatch then tempStats.Level = lvlMatch end

            if txt:match("^%d+$") or txt:match("^%d+,%d+$") then
                if name:find("coin") or pName:find("coin") or name:find("point") or pName:find("point") then tempStats.Coins = txt
                elseif name:find("gem") or pName:find("gem") or name:find("diamond") then tempStats.Gems = txt
                elseif name:find("level") or pName:find("level") or name:find("lvl") then tempStats.Level = txt end
            end
        end
    end
    ...
    if tempStats.Level == "0" or tempStats.Level == LiveStats.Level then
        for _, obj in ipairs(pGui:GetDescendants()) do
            if obj:IsA("TextLabel") and obj.Text:match("^%d+$") and obj.Visible then
                local pName = obj.Parent and obj.Parent.Name:lower() or ""
                local gName = obj.Parent and obj.Parent.Parent and obj.Parent.Parent.Name:lower() or ""
                if pName:find("star") or pName:find("level") or pName:find("lvl") or gName:find("star") then tempStats.Level = obj.Text end
            end
        end
        if tempStats.Level == LiveStats.Level then
            for _, obj in ipairs(pGui:GetDescendants()) do
                if obj:IsA("TextLabel") and (obj.Text:match("%d+/%d+ XP") or obj.Text:match("XP")) and obj.Parent then
                    for _, sib in ipairs(obj.Parent:GetChildren()) do
                        if sib:IsA("TextLabel") and sib.Text:match("^%d+$") and sib ~= obj then tempStats.Level = sib.Text end
                    end
                end
            end
        end
    ...
    LiveStats.Level = tempStats.Level; LiveStats.XP = tempStats.XP; LiveStats.Coins = tempStats.Coins; LiveStats.Gems = tempStats.Gems; LiveStats.Rebirths = tempStats.Rebirths
end
task.spawn(function() while task.wait(1) do pcall(ScrapeFE2Data) end end)
...
function StartAutoQueue()
    ...
    AutoQueueListener = Multiplayer.ChildAdded:Connect(function(newMap)
        ...
        -- Cek Neural AI: Kalo AutoQueue nyala tapi script lu mati (lobby observer), mulai rekam!
        if CONFIG.NEURAL_PATHFINDER and not Check("InGame") then
            task.spawn(function() StartObserverMode(mapName) end)
        end
        
        local timeout = os.clock() + 15 
        repeat task.wait(0.1) until Check("InGame") or not STATE.AUTO_QUEUE_ENABLED or os.clock() > timeout
        
        -- Kalo kita berhasil masuk map
        if Check("InGame") and observing then StopObserver(false, mapName) end -- Stop observe kalo kita main
        
        if not STATE.AUTO_QUEUE_ENABLED then STATE.HybridActive = false; return end
        STATE.mapCompleted = false
        if not Check("InGame") then
            notify("⏰ Timeout 15s (" .. mapName .. "). Fallback Auto Farm.", "Hybrid System")
            getgenv().TomatoAutoFarm = true; STATE.CurrentlyFarming = true
            StopWinEngine(); StartWinEngine(); STATE.HybridActive = false; return
        end
        task.wait(0.5)
        if isMapInTAS(mapName) then
            ...
            notify("⚡ TAS Tidak Support (" .. mapName .. ")! Switch Auto Farm...", "Hybrid System")
            getgenv().TomatoAutoFarm = true; STATE.CurrentlyFarming = true
...
local API = {
    ...
    notify = notify, UIHooks = {}, LiveStats = LiveStats, NeuralData = _G.NeuralData
}
...
if success and uiScript then local runUI, err = loadstring(uiScript); if runUI then runUI() else warn("UI Error:", err) end end
...
local function applyWatermark()
    ...
    txt.Size = UDim2.new(1, 0, 0, 25); txt.Position = UDim2.new(0.5, 0, 0.5, 0); txt.AnchorPoint = Vector2.new(0.5, 0.5)
    txt.BackgroundTransparency = 1; txt.Text = "💧 TROXZY VIP - FLOOD ESCAPE 2"
    txt.TextColor3 = Color3.fromRGB(255, 255, 255); txt.TextTransparency = 0.8
    txt.TextSize = 25; txt.Font = Enum.Font.GothamBlack
end
    ...
    if CONFIG.SPEED and not STATE.CurrentlyFarming then
        ...
        if char then
            ...
            if hum and root and hum.MoveDirection.Magnitude > 0 then
                ...
                if speedBoost > 0 then root.CFrame = root.CFrame + (hum.MoveDirection * speedBoost) end
            end
        end
    end
end))

local lastHeartbeat, slowTick = 0, 0
TrackConnection(RunService.Heartbeat:Connect(function()
    ...
    updateAdminDetection(); applyWatermark()
    slowTick = slowTick + 1
    if slowTick > 20 then antiSpyLoop(); slowTick = 0 end
    if API.UIHooks.updateDashboard then pcall(API.UIHooks.updateDashboard) end
end))

TrackConnection(RunService.Heartbeat:Connect(function() pcall(updateESP) end))
...
TrackConnection(UIS.JumpRequest:Connect(function() if CONFIG.INF_JUMP and Player.Character then local h = Player.Character:FindFirstChild("Humanoid"); if h then h:ChangeState(Enum.HumanoidStateType.Jumping); h.Jump = true end end end))
