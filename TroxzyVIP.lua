-- ============================================
-- TROXZY VIP v24.18 "SPECTRAL BLADE" [BACKEND CORE - OVERDRIVED]
-- Badan Intelijen Negara - STRICT ADMIN DETECT + HYBRID FIX + LOBBY TRACKER
-- ============================================
...
-- 🔥 SUPERIOR ADMIN DETECTION (STRICT LOGIC BY BIN) 🔥
local function isAdmin(p)
    if not p then return false end

    -- 1. Cek Game Creator (Pasti punya akses Admin/Owner)
    if p.UserId == game.CreatorId then return true end
    
    -- 2. Cek Staff Roblox Resmi (Grup ID 1200769)
    ...
    -- 3. Tracker UI Admin Framework Populer (Adonis, HD Admin, Kohl's)
    local pGui = p:FindFirstChild("PlayerGui")
    if pGui then
        if pGui:FindFirstChild("HDAdminGUIs") or 
           pGui:FindFirstChild("Adonis_UI") or 
           pGui:FindFirstChild("KohlsAdmin") or
           pGui:FindFirstChild("AdminUI") then
            return true
        end
    end

    -- 4. Cek Strict Leaderstats (Murni baca data server rank)
    ...
    if leaderstats then
        for _, stat in ipairs(leaderstats:GetChildren()) do
            ...
            if lowerName == "admin" or lowerName == "mod" or lowerName == "rank" or lowerName == "role" or lowerName == "staff" then
                ...
                if val == "admin" or val == "mod" or val == "owner" or val == "dev" or val == "staff" or val == "creator" or val == "headadmin" then 
                    return true 
                end
            end
        end
...
-- =======================================================
-- 🔥 HYBRID AUTO-QUEUE (FIXED & OVERDRIVED BY BIN) 🔥
-- =======================================================
