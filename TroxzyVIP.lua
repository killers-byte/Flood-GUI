-- ============================================
-- TROXZY VIP - FLOOD ESCAPE 2 v16.2 [POLISHED + PATCHED]
-- 🔥 TAS URLs updated to forked repo
...
local function notify(msg, title)
    pcall(function() StarterGui:SetCore("SendNotification", {Title = title or "Troxzy", Text = msg, Duration = 2}) end)
end
...
local function saveStats()
    ...
    writefile("Troxzy_Stats.json", HttpService:JSONEncode(Stats)); Stats.sessionStart = tick()
end
...
local function rotateMap()
    ...
    rotationIndex = rotationIndex + 1; if rotationIndex > #MapRotation then rotationIndex = 1 end
end
...
local function autoLeaveIfAdmin()
    ...
    notify("Admin detected! Leaving...", "Warning"); saveStats(); task.wait(1)
...
-- Random Delay
local buttonCount = 0
local function getRandomFarmDelay() return CONFIG.RANDOM_DELAY and math.random(10,50)/10 or 0 end
...
-- ==================== PATCHED: TAS URLs dari fork killers-byte ====================
...
local ReqPasskey, NewMapVote, UpdMapVote, AddedWaiting, AlertRemote =
    RemoteFolder:WaitForChild("ReqPasskey"), RemoteFolder:WaitForChild("NewMapVote"),
    RemoteFolder:WaitForChild("UpdMapVote"), RemoteFolder:WaitForChild("AddedWaiting"),
...
local function Check(flag)
    ...
    elseif flag == "InGame" then return hrp.Position.X > 50 end; return false
end
local function GetRandomPointInPart(part)
    local s = part.Size; return part.CFrame * CFrame.new((math.random()-0.5)*(s.X*0.9), (math.random()-0.5)*(s.Y*0.9), (math.random()-0.5)*(s.Z*0.9))
end
local function GetCurrentDifficultyRank()
    local ok, res = pcall(function()
        return string.gsub(string.split(Workspace.Lobby.GameInfo.SurfaceGui.Frame.Difficulty.Difficulty.Text, ":")[1], "^%s*(.-)%s*$", "%1")
    end)
    if ok and res then return DIFFICULTY_RANKS[res] or 0, res end; return 0, "Unknown"
end
local function isRandomString(str)
    if #str == 0 then return false end
    for i = 1, #str do if str:sub(i,i):lower() == str:sub(i,i) then return false end end; return true
end

-- Noclip
local noclipCache, noclipActive = {}, false
local function refreshNoclipCache()
    noclipCache = {}; local char = GetChar()
    if char then for _, v in pairs(char:GetDescendants()) do if v:IsA("BasePart") then table.insert(noclipCache, v) end end end
end
local function applyNoclip(state)
    if state == noclipActive then return end; noclipActive = state
    for _, v in ipairs(noclipCache) do if v and v.Parent then v.CanCollide = not state end end
end
...
-- ESP
local lastESPUpdate, espCache = 0, {}
local function clearESPCache()
    for player, highlight in pairs(espCache) do pcall(function() highlight:Destroy() end) end
    espCache = {}; lastESPUpdate = 0
end
local function updateESP()
    local now = tick(); if now - lastESPUpdate < 0.3 then return end; lastESPUpdate = now
    local toRemove = {}
    for p, hl in pairs(espCache) do
        if not p or not p.Parent or not p.Character or not p.Character:FindFirstChild("Head") then
            table.insert(toRemove, p); pcall(function() hl:Destroy() end)
        end
    end
    for _, p in ipairs(toRemove) do espCache[p] = nil end
    if not CONFIG.ESP then for _, hl in pairs(espCache) do pcall(function() hl:Destroy() end) end; espCache = {}; return end
    for _, p in pairs(Players:GetPlayers()) do
        if p ~= Player and p.Character and p.Character:FindFirstChild("Head") and not espCache[p] then
            local hl = p.Character:FindFirstChild("PlayerHighlight")
            if not hl then hl = Instance.new("Highlight", p.Character); hl.Name = "PlayerHighlight"; hl.FillColor = Color3.fromRGB(160, 180, 200) end
            espCache[p] = hl
        end
    end
end
...
local function updateVisuals()
    if tick() - lastVisualUpdate < 0.5 then return end; lastVisualUpdate = tick()
    Lighting.Brightness = CONFIG.FULLBRIGHT and 2 or 1; Lighting.FogEnd = CONFIG.FULLBRIGHT and 99999 or 10000
    if Camera then
        local tfov = CONFIG.FOV and CONFIG.FOV_VAL or 70
        if tfov ~= lastFOVValue then Camera.FieldOfView = tfov; lastFOVValue = tfov end
    end
end
...
local function OnMapLoad(map)
    ...
    if isMapBlacklisted(mapName) then
        ...
        local char = GetChar()
        if char then local hrp = char:FindFirstChild("HumanoidRootPart"); local hum = char:FindFirstChild("Humanoid")
    ...
    for _, obj in pairs(map:GetDescendants()) do
        if isRandomString(obj.Name) and obj.ClassName == "Model" then
            ...
            for _, c in pairs(obj:GetChildren()) do
                if c:IsA("BasePart") and tostring(c.BrickColor) ~= "Medium stone grey" then hitbox = c; break end
            end
        ...
        if not exitRegion then
            ...
            for _, button in pairs(buttons) do
                ...
                if bh and button:FindFirstChild("TouchInterest", true) and button:FindFirstChildWhichIsA("BillboardGui", true) then
                    failedScan = false; currentHRP.Anchored = false
                    currentHRP.CFrame = CFrame.new(bh.Position - stealthOffset())
                    hum:ChangeState(Enum.HumanoidStateType.Jumping); task.wait(stealthDelay())
                    hum:ChangeState(Enum.HumanoidStateType.Running); task.wait(stealthDelay())
                end
            end
            if failedScan then RunService.Heartbeat:Wait() end
        else
            applyNoclip(false); currentHRP.Anchored = false
            if Camera.CameraSubject ~= exitRegion then Camera.CameraSubject = exitRegion end
            if not Escaped then
                currentHRP.CFrame = GetRandomPointInPart(exitRegion); currentHRP.Velocity = Vector3.zero
...
-- Vote System
local function GetSessionKey() local s,k = pcall(function() return ReqPasskey:InvokeServer() end); return s and k and -k or nil end
local function CalculateCost(md, mv)
    local c, id = 0, 0
    if mv then c = mv.voteCount or 0; id = mv.mapID or 0 end
    if id == md.ID then return math.clamp(md.extraVoteCost + (c-1)*10, 0, 50) end
    return md.locked and md.unlockCost or 0
end
TrackConnection(NewMapVote.OnClientEvent:Connect(function(d)
    local maps, pVotes = d.mapData, d.pVotes or {}; if not maps then return end
    local ft
    ...
    if ft then
        ...
        if CONFIG.TAS_AUTO_START then task.spawn(ExecuteTAS) else notify("TAS Auto-Start is OFF.", "Info") end
    end
end))
...
TitleLabel.Size = UDim2.new(0,110,0,20); TitleLabel.Position = UDim2.new(1,-120,0.5,-14)
TitleLabel.Text = "Troxzy VIP v16"; TitleLabel.TextColor3 = Color3.fromRGB(180,200,220)
...
UserID.Size = UDim2.new(0,110,0,12); UserID.Position = UDim2.new(1,-120,0.5,4)
...
StatsBar.Size = UDim2.new(1,-16,0,22); StatsBar.Position = UDim2.new(0,8,0,56)
...
StatsLabel.Font = Enum.Font.GothamMedium; StatsLabel.TextSize = 10
...
TabList.FillDirection = Enum.FillDirection.Horizontal; TabList.Padding = UDim.new(0,2); TabList.HorizontalAlignment = Enum.HorizontalAlignment.Center
...
for i, tab in ipairs(tabItems) do
    ...
    tabBtn.Size = UDim2.new(0.155,-2,1,0)
    tabBtn.BackgroundColor3 = (tab.key=="Farm") and Color3.fromRGB(40,50,65) or Color3.fromRGB(25,25,33)
    tabBtn.Text = tab.name; tabBtn.TextSize = 8; tabBtn.Font = Enum.Font.GothamBold
...
local function AddSection(tabKey, title)
    ...
    s.Font = Enum.Font.GothamBold; s.TextSize = 9; s.BackgroundTransparency = 1; s.TextXAlignment = Enum.TextXAlignment.Left; updateScrollSize(tabKey)
end
...
local function AddInfoLabel(tabKey, text)
    ...
    l.TextColor3 = Color3.fromRGB(180,210,255); l.Font = Enum.Font.GothamMedium; l.TextSize = 11; l.BorderSizePixel = 0; addCorner(l,6); updateScrollSize(tabKey); return l
end
...
local function AddToggle(tabKey, name, stateKey)
    ...
    local lbl = Instance.new("TextLabel", f); lbl.Size = UDim2.new(0.45,0,1,0); lbl.Position = UDim2.new(0,10,0,0)
    lbl.Text = name; lbl.TextColor3 = Color3.fromRGB(220,225,235); lbl.Font = Enum.Font.GothamMedium; lbl.TextSize = 11
...
local function applyTheme(theme)
    local t = theme == "Dark" and DARK_THEME or LIGHT_THEME; currentTheme = theme
...
local function createDashboardContent()
    ...
    statusLabel.Font = Enum.Font.GothamBold; statusLabel.TextSize = 9; statusLabel.BackgroundTransparency = 1; statusLabel.Name = "StatusLabel"
end
createDashboardContent()

local function updateDashboard()
    if not Dashboard or not Dashboard.Visible then return end
    local mapLabel = Dashboard:FindFirstChild("MapLabel"); local timeLabel = Dashboard:FindFirstChild("TimeLabel")
    local speedLabel = Dashboard:FindFirstChild("SpeedLabel"); local statusLabel = Dashboard:FindFirstChild("StatusLabel")
    ...
    if speedLabel then local hours = (tick() - Stats.sessionStart) / 3600; speedLabel.Text = "Maps/hr: " .. (hours > 0 and math.floor(Stats.mapsCompleted / hours) or 0) end
    if statusLabel then
        if panicActive then statusLabel.Text = "Status: PANIC"; statusLabel.TextColor3 = Color3.fromRGB(255,50,50)
        elseif CurrentlyFarming then statusLabel.Text = "Status: Farming"; statusLabel.TextColor3 = Color3.fromRGB(100,255,100)
        elseif CONFIG.STEALTH_MODE then statusLabel.Text = "Status: Stealth"; statusLabel.TextColor3 = Color3.fromRGB(255,200,50)
        else statusLabel.Text = "Status: Idle"; statusLabel.TextColor3 = Color3.fromRGB(160,160,160) end
    end
end

-- Build Menu
AddSection("Farm","AUTO FARM"); AddToggle("Farm","Auto Farm","AutoFarm"); AddInfoLabel("Farm","[Target] "..CONFIG.TARGET_MAP)
AddSection("TAS","TAS LOADER")
AddToggle("TAS","TAS Auto-Start","TAS_AUTO_START")
AddButton("TAS","Record Mode",Color3.fromRGB(180,40,40),function() CONFIG.TAS_MODE="Record"; if CONFIG.TAS_AUTO_START then ExecuteTAS() else notify("Enable TAS Auto-Start first", "TAS") end end)
AddButton("TAS","Play Mode",Color3.fromRGB(30,160,50),function() CONFIG.TAS_MODE="Play"; if CONFIG.TAS_AUTO_START then ExecuteTAS() else notify("Enable TAS Auto-Start first", "TAS") end end)
AddSection("Move","MOVEMENT"); AddToggle("Move","Noclip","NOCLIP"); AddToggle("Move","Speed Boost","SPEED"); AddToggle("Move","Infinite Jump","INF_JUMP"); AddInput("Move","Speed",CONFIG.SPEED_VAL,function(v) CONFIG.SPEED_VAL=v end)
AddSection("Visual","COMBAT + RENDER"); AddToggle("Visual","God Mode","GOD_MODE"); AddToggle("Visual","ESP","ESP"); AddToggle("Visual","Fullbright","FULLBRIGHT"); AddToggle("Visual","FOV Changer","FOV"); AddInput("Visual","FOV",CONFIG.FOV_VAL,function(v) CONFIG.FOV_VAL=v end)
AddSection("Stealth","ANTI-DETECTION")
AddToggle("Stealth","Stealth Mode","STEALTH_MODE"); AddToggle("Stealth","Admin Detector","ADMIN_DETECTOR")
AddToggle("Stealth","Auto-Leave Admin","AUTO_LEAVE_ADMIN"); AddToggle("Stealth","Random Delay","RANDOM_DELAY")
AddToggle("Stealth","Hide Script","HIDE_SCRIPT"); AddToggle("Stealth","Map Rotation","MAP_ROTATION")
AddButton("Stealth","Force Leave",Color3.fromRGB(180,50,50),function() forceReconnect() end)
AddSection("Premium","PREMIUM FEATURES")
AddToggle("Premium","Live Dashboard","DASHBOARD"); AddToggle("Premium","Smart Alerts","SMART_ALERTS")
AddToggle("Premium","Night Mode","NIGHT_MODE"); AddToggle("Premium","Auto-Updater","AUTO_UPDATE")
...
TrackConnection(RunService.Heartbeat:Connect(function()
    local now = tick(); if now - lastHeartbeat < 0.1 then return end; lastHeartbeat = now
    pcall(function()
        local char = Player.Character; if not char then return end
        local hum = char:FindFirstChild("Humanoid"); if not hum then return end
        if CONFIG.NOCLIP and not CurrentlyFarming then applyNoclip(true) elseif not CurrentlyFarming then applyNoclip(false) end
        ...
        if hum:GetState() == Enum.HumanoidStateType.Swimming then hum:ChangeState(Enum.HumanoidStateType.Landed); hum.PlatformStand=false; task.wait(0.05); hum:ChangeState(Enum.HumanoidStateType.Jumping) end
    end)
end))

TrackConnection(RunService.Heartbeat:Connect(function() if tick() - lastESPUpdate < 0.3 then return end; pcall(updateESP) end))
TrackConnection(RunService.Heartbeat:Connect(function() if tick() - lastVisualUpdate < 0.5 then return end; pcall(updateVisuals) end))

task.spawn(function() while task.wait(2) do pcall(updateDashboard) end end)

UIS.JumpRequest:Connect(function()
    if CONFIG.INF_JUMP and Player.Character then local hum = Player.Character:FindFirstChild("Humanoid"); if hum then hum:ChangeState(Enum.HumanoidStateType.Jumping) end end
end)
...
print("Troxzy VIP v16.2 - Polished + PATCHED (forked TAS URLs)")
print("Fork: killers-byte/Flood-GUI")
