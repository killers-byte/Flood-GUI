...
-- 🔥 PROFESSIONAL UI + FULL CORE FEATURES
-- 🔥 Auto Farm, TAS Injector, Smart Stealth
-- ============================================
...
local DARK_THEME = {
    MainBg = Color3.fromRGB(17, 17, 26),
...
local LIGHT_THEME = {
    ...
    TabActive = Color3.fromRGB(210, 210, 230),
    TabInactive = Color3.fromRGB(230, 230, 240),
    TextBright = Color3.fromRGB(20, 20, 30),
    TextMedium = Color3.fromRGB(60, 60, 80),
    TextDim = Color3.fromRGB(100, 100, 120),
    StatsBg = Color3.fromRGB(220, 220, 235),
    StatsText = Color3.fromRGB(30, 50, 100),
    SectionText = Color3.fromRGB(80, 80, 100),
    ToggleBg = Color3.fromRGB(200, 200, 220),
    ToggleDot = Color3.fromRGB(150, 150, 170),
    InputBg = Color3.fromRGB(220, 220, 235),
    InfoBg = Color3.fromRGB(210, 210, 230),
    InfoText = Color3.fromRGB(20, 40, 80),
...
if CONFIG.ANTI_REPORT then preventReports() end

local floodColorMap = {
    Blue = Color3.fromRGB(0, 150, 255), Green = Color3.fromRGB(0, 255, 100),
    Red = Color3.fromRGB(255, 50, 50), Pink = Color3.fromRGB(255, 100, 200),
...
local function toggleTASPause()
    ...
    if TAS_STATUS_LABEL then
        TAS_STATUS_LABEL.Text = "Status: " .. (CONFIG.TAS_PAUSED and "⏸ PAUSED" or "▶ RESUMING...")
    end
...
local function refreshNoclip()
    ...
    if char then
        for _, v in pairs(char:GetDescendants()) do
            if v:IsA("BasePart") then table.insert(ncCache, v) end
        end
    end
end

local function applyNoclip(state)
    if state == ncActive then return end
    ncActive = state
    for _, v in ipairs(ncCache) do
...
local function deactivatePanicMode()
    ...
    if Main then 
        Main.Visible = true
        Tween(Main, {Size = UDim2.new(0,390,0,540)}, 0.4) 
    end
...
local function updateVisuals()
    ...
    if Camera then
        ...
        if tfov ~= lastFOV then
            Tween(Camera, {FieldOfView = tfov})
            lastFOV = tfov
        end
    end
...
local function OnMapLoad(map)
    ...
    if isMapBlacklisted(Stats.currentMap) then
        ...
        pcall(function()
            ...
            c.HumanoidRootPart.CFrame = CFrame.new(1000, 1000, 1000)
            task.wait(0.25)
            c.Humanoid.Health = 0
        end)
        CurrentlyFarming = false
    ...
    if CONFIG.COLLECT_ITEMS then
        local lp = map:FindFirstChild("_LostPage", true)
        local rsc = map:FindFirstChild("_Rescue", true)
        if lp then
            hrp.CFrame = lp.CFrame
            task.wait()
            hrp.CFrame = hrp.CFrame + Vector3.new(0, 5, 0)
        end
        if rsc then
            hrp.CFrame = rsc.Contact.CFrame
            task.wait()
            hrp.CFrame = hrp.CFrame + Vector3.new(0, 5, 0)
        end
    end
    ...
    for _, obj in pairs(map:GetDescendants()) do
        if isRandStr(obj.Name) and obj.ClassName == "Model" then
            ...
            if hb and isRandStr(hb.Name) then
                hb.Name = "Hitbox"
    ...
    if CONFIG.GOD_MODE then
        godCon = hum:GetPropertyChangedSignal("Health"):Connect(function()
    ...
    while RunService.Heartbeat:Wait() and Check("InGame") and _G.TroxzyAutoFarm and not panicActive do
        ...
        if CONFIG.HIDE_SCRIPT and Main then
            Tween(Main, {BackgroundTransparency = isPlayerNearby() and 1 or 0}, 0.5)
        end

        if shouldTakeBreak() then
            task.wait(stealthDelay() * 10)
        end
        ...
        if not er then
            ...
            for _, btnModel in pairs(btns) do
                ...
                if bh and btnModel:FindFirstChild("TouchInterest", true) and btnModel:FindFirstChildWhichIsA("BillboardGui", true) then
                    ...
                    currHRP.CFrame = CFrame.new(bh.Position - stealthOffset())
                    hum:ChangeState(Enum.HumanoidStateType.Jumping)
            ...
            if not Escaped then
                currHRP.CFrame = GetRandomPoint(er)
...
TrackConnection(NewMapVote.OnClientEvent:Connect(function(data)
    ...
    if targetMap then
        ...
        if CONFIG.TAS_AUTO_START then task.spawn(ExecuteTAS) end
    end
end))
...
hc.Size = UDim2.new(1,0,0.5,0); hc.Position = UDim2.new(0,0,0.5,0)
hc.BackgroundColor3 = DARK_THEME.HeaderBg; hc.BorderSizePixel = 0; hc.Parent = Header
...
task.spawn(function() 
    while task.wait(5) do pcall(function() StatsLabel.Text = getStatsText() end) end 
end)
...
local tabItems = { 
    {name="Farm", key="Farm"}, {name="TAS", key="TAS"}, 
    {name="Move", key="Move"}, {name="Visual", key="Visual"}, 
    {name="Stealth", key="Stealth"}, {name="Extra", key="Extra"} 
}
...
for i, tab in ipairs(tabItems) do
    ...
    tabBtn.MouseEnter:Connect(function() 
        if tabContents[i].scroll.Parent.Visible == false then Tween(tabBtn, {BackgroundColor3 = Color3.fromRGB(35,35,50)}) end 
    end)
    tabBtn.MouseLeave:Connect(function() 
        if tabContents[i].scroll.Parent.Visible == false then Tween(tabBtn, {BackgroundColor3 = DARK_THEME.TabInactive}) end 
    end)
end
...
local function AddSection(tabKey, title)
    ...
    s.Size = UDim2.new(1,0,0,24); s.Text = "  " .. title
    s.TextColor3 = DARK_THEME.Accent; s.Font = Enum.Font.GothamBlack; s.TextSize = 11
...
local function AddButton(tabKey, name, color, callback)
    ...
    b.Size = UDim2.new(1,0,0,38); b.BackgroundColor3 = color; b.Text = name
    b.TextSize = 12; b.Font = Enum.Font.GothamBold; b.TextColor3 = Color3.fromRGB(255,255,255)
    b.AutoButtonColor = false; addCorner(b,6); addStroke(b, Color3.new(1,1,1), 1, 0.9)
    ...
    b.MouseButton1Click:Connect(function() 
        Tween(b, {Size = UDim2.new(0.98,0,0,34)}, 0.1)
        task.wait(0.1)
        Tween(b, {Size = UDim2.new(1,0,0,38)}, 0.1)
        pcall(callback) 
    end)
    return b
end
...
local function AddInfoLabel(tabKey, text)
    ...
    l.Size = UDim2.new(1,0,0,36); l.BackgroundColor3 = DARK_THEME.InfoBg
    l.Text = text; l.TextColor3 = DARK_THEME.InfoText; l.Font = Enum.Font.GothamMedium; l.TextSize = 11
    addCorner(l,6); addStroke(l, DARK_THEME.Border, 1, 0.9)
...
local function AddToggle(tabKey, name, stateKey)
    ...
    f.Size = UDim2.new(1,0,0,40); f.BackgroundColor3 = DARK_THEME.ToggleBg
    addCorner(f,6); addStroke(f, DARK_THEME.Border, 1, 0.9)
    ...
    lbl.Text = name; lbl.TextColor3 = DARK_THEME.TextMedium; lbl.Font = Enum.Font.GothamMedium
    lbl.TextSize = 12; lbl.BackgroundTransparency = 1; lbl.TextXAlignment = Enum.TextXAlignment.Left
    lbl.Parent = f
    
    local sb = Instance.new("Frame")
    sb.Size = UDim2.new(0,40,0,20); sb.Position = UDim2.new(1,-54,0.5,-10)
    sb.BackgroundColor3 = DARK_THEME.ToggleBg; addCorner(sb,10); addStroke(sb, DARK_THEME.Border, 1, 0.8)
    sb.Parent = f
    
    local dot = Instance.new("Frame")
    dot.Size = UDim2.new(0,14,0,14); dot.Position = UDim2.new(0,3,0.5,-7)
    dot.BackgroundColor3 = DARK_THEME.ToggleDot; addCorner(dot,7)
    dot.Parent = sb
    ...
    if stateKey then 
        _G.ToggleStates[stateKey] = { state = false, dot = dot, sb = sb, btn = btn, SetState = setToggleUI } 
    end
    ...
    btn.MouseButton1Click:Connect(function()
        ...
        elseif stateKey == "DASHBOARD" then 
            ...
            if _G.DashboardUI then _G.DashboardUI.Visible = state end
        ...
        elseif stateKey == "AUTO_QUEUE" then 
            ...
            notify("Auto Queue " .. (state and "ON" or "OFF"))
        else 
...
local function AddInput(tabKey, label, defaultVal, callback)
    local tabIdx; for i, t in ipairs(tabItems) do if t.key==tabKey then tabIdx=i; break end end; if not tabIdx then return end
    local f = Instance.new("Frame")
    f.Size = UDim2.new(1,0,0,44); f.BackgroundColor3 = DARK_THEME.ToggleBg
    addCorner(f,6); addStroke(f, DARK_THEME.Border, 1, 0.9)
    f.Parent = tabContents[tabIdx].scroll
    RegisterThemeObject(f, "BackgroundColor3", DARK_THEME.ToggleBg, LIGHT_THEME.ToggleBg)
    ...
    lbl.Text = label; lbl.TextColor3 = DARK_THEME.TextMedium; lbl.Font = Enum.Font.GothamMedium
    lbl.TextSize = 12; lbl.BackgroundTransparency = 1; lbl.TextXAlignment = Enum.TextXAlignment.Left
    lbl.Parent = f
    
    local inp = Instance.new("TextBox")
    inp.Size = UDim2.new(0,70,0,26); inp.Position = UDim2.new(1,-84,0.5,-13)
    inp.BackgroundColor3 = DARK_THEME.InputBg; inp.TextColor3 = DARK_THEME.TextBright
    inp.PlaceholderText = tostring(defaultVal); inp.Text = tostring(defaultVal)
    inp.Font = Enum.Font.GothamBold; inp.TextSize = 11
    addCorner(inp,6); addStroke(inp, DARK_THEME.Border, 1, 0.8)
    inp.Parent = f
    
    inp.FocusLost:Connect(function() 
        local v = tonumber(inp.Text)
        if v then callback(v) else inp.Text = tostring(defaultVal) end 
    end)
end

-- ==================== DASHBOARD PANEL ====================
local Dashboard = Instance.new("Frame")
Dashboard.Size = UDim2.new(0,210,0,95)
...
dTitle.Size = UDim2.new(1,0,0,24); dTitle.Text = " OVERVIEW"; dTitle.TextColor3 = DARK_THEME.Accent
dTitle.Font = Enum.Font.GothamBlack; dTitle.TextSize = 11; dTitle.BackgroundTransparency = 1
dTitle.TextXAlignment = Enum.TextXAlignment.Left; dTitle.Parent = Dashboard

local mapLabel = Instance.new("TextLabel")
mapLabel.Size = UDim2.new(1,-10,0,16); mapLabel.Position = UDim2.new(0,10,0,26)
mapLabel.Text = "Map: Waiting..."; mapLabel.TextColor3 = DARK_THEME.TextBright
mapLabel.Font = Enum.Font.GothamMedium; mapLabel.TextSize = 10; mapLabel.BackgroundTransparency = 1
mapLabel.TextXAlignment = Enum.TextXAlignment.Left; mapLabel.Parent = Dashboard

local timeLabel = Instance.new("TextLabel")
timeLabel.Size = UDim2.new(1,-10,0,16); timeLabel.Position = UDim2.new(0,10,0,42)
timeLabel.Text = "Time: 0m"; timeLabel.TextColor3 = DARK_THEME.TextMedium
timeLabel.Font = Enum.Font.Gotham; timeLabel.TextSize = 10; timeLabel.BackgroundTransparency = 1
timeLabel.TextXAlignment = Enum.TextXAlignment.Left; timeLabel.Parent = Dashboard

local speedLabel = Instance.new("TextLabel")
speedLabel.Size = UDim2.new(1,-10,0,16); speedLabel.Position = UDim2.new(0,10,0,58)
speedLabel.Text = "Rate: 0 maps/hr"; speedLabel.TextColor3 = DARK_THEME.TextMedium
speedLabel.Font = Enum.Font.Gotham; speedLabel.TextSize = 10; speedLabel.BackgroundTransparency = 1
speedLabel.TextXAlignment = Enum.TextXAlignment.Left; speedLabel.Parent = Dashboard

local statusLabel = Instance.new("TextLabel")
statusLabel.Size = UDim2.new(1,-10,0,16); statusLabel.Position = UDim2.new(0,10,0,74)
statusLabel.Text = "Status: Idle"; statusLabel.TextColor3 = Color3.fromRGB(0,230,120)
statusLabel.Font = Enum.Font.GothamBold; statusLabel.TextSize = 10; statusLabel.BackgroundTransparency = 1
statusLabel.TextXAlignment = Enum.TextXAlignment.Left; statusLabel.Parent = Dashboard
...
local function updateDashboard()
    ...
    if panicActive then 
        statusLabel.Text = "Status: PANIC"; statusLabel.TextColor3 = Color3.fromRGB(255,80,80) 
    elseif CurrentlyFarming then 
        statusLabel.Text = "Status: Farming"; statusLabel.TextColor3 = Color3.fromRGB(0,230,120) 
    elseif CONFIG.STEALTH_MODE then 
        statusLabel.Text = "Status: Stealth"; statusLabel.TextColor3 = Color3.fromRGB(255,180,50) 
    else 
        statusLabel.Text = "Status: Idle"; statusLabel.TextColor3 = DARK_THEME.TextDim 
    end
end
...
AddButton("TAS", "Record Route", DARK_THEME.ButtonRecord, function() 
    if not CONFIG.TAS_AUTO_START then notify("Enable Auto-Start"); return end
    CONFIG.TAS_MODE="Record"; task.spawn(ExecuteTAS) 
end)
AddButton("TAS", "Play Route", DARK_THEME.Accent, function() 
    if not CONFIG.TAS_AUTO_START then notify("Enable Auto-Start"); return end
    CONFIG.TAS_MODE="Play"; task.spawn(ExecuteTAS) 
end)
...
AddButton("Stealth", "Panic Mode [P]", DARK_THEME.ButtonPanic, function() 
...
AddButton("Extra", "Cycle Color", DARK_THEME.Accent, function() 
    local c={"Blue","Green","Red","Pink","Purple"}
    local idx=table.find(c,CONFIG.FLOOD_COLOR)
    idx=idx and (idx%#c)+1 or 1
    CONFIG.FLOOD_COLOR=c[idx]
    applyFloodColors()
    FCLabel.Text="Current: "..CONFIG.FLOOD_COLOR 
end)
...
TrackConnection(RunService.Heartbeat:Connect(function()
    if os.clock() - lastHeartbeat < 0.1 then return end
...
task.spawn(function() 
    while task.wait(1) do pcall(updateDashboard) end 
end)
...
task.spawn(function() 
    while task.wait(10) do handleAdminDetection() end 
end)

task.spawn(function() 
    while task.wait(1) do 
        if _G.TroxzyAutoFarm and Player.Character and Player.Character:FindFirstChild("HumanoidRootPart") and not Check("InLift") and not Check("InGame") then 
            AddedWaiting:FireServer() 
        end 
    end 
end)
...
notify("Professional UI + Core Loaded!", "Welcome")
print("Troxzy VIP - Professional UI Edition (Full Features) active.")
