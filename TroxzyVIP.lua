...
-- 🔥 FIXED: AUTO REBIRTH SYSTEM 2.0 (OBFUSCATOR SAFE)
-- ============================================
...
function StartAutoQueue()
    if AutoQueueListener then AutoQueueListener:Disconnect() end; STATE.moveToLift = false; STATE.mapCompleted = false
    AutoQueueListener = Multiplayer.ChildAdded:Connect(function(newMap)
        ...
        if not STATE.AUTO_QUEUE_ENABLED then return end; STATE.mapCompleted = false
        if not STATE.TAS_RUNNING then getgenv().TomatoAutoFarm = false; task.spawn(ExecuteTAS) end
    end)
    STATE.AUTO_QUEUE_ENABLED = true
end
function StopAutoQueue() if AutoQueueListener then AutoQueueListener:Disconnect(); AutoQueueListener = nil end; STATE.moveToLift = false; STATE.AUTO_QUEUE_ENABLED = false; if STATE.TAS_RUNNING and TAS_COROUTINE then pcall(coroutine.close, TAS_COROUTINE); STATE.TAS_RUNNING = false end end
...
local REBIRTH_CONFIG = {
    ENABLED = true, -- FIX: Dinyalakan secara otomatis!
    CHECK_INTERVAL = 3,
    AUTO_RESUME = true, 
    MAX_RETRIES = 5,
    REBIRTH_DELAY_MIN = 2, 
    ...
    -- ===== ⚙️ AREA KONFIGURASI GAME ⚙️ =====
    MAX_LEVEL = 100,           -- <==== UBAH INI: Angka target untuk Rebirth
    LEADERSTAT_NAME = "Level", -- <==== UBAH INI: Jika di game namanya "Wins" / "Coins", ganti di sini
    -- ======================================
...
-- FIX: Deep Search Remote di seluruh ReplicatedStorage
...
-- FIX: Deteksi Angka Leaderstat kebal Teks/String (contoh: "1,000" jadi 1000)
    ...
    if RebirthState.foundRemote then 
        local success = pcall(function() 
            if RebirthState.foundRemote:IsA("RemoteEvent") or RebirthState.foundRemote:IsA("BindableEvent") then 
                ...
                RebirthState.foundRemote:FireServer(1) -- Pancingan jika server butuh parameter
...
-- FIX: Otomatis menjalankan fitur pemantauan Auto Rebirth saat script ter-load
if REBIRTH_CONFIG.ENABLED then StartAutoRebirth() end

notify("TROXZY VIP - BACKEND + AUTO REBIRTH V2 LOADED", "System")
