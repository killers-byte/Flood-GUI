...
-- 🔥 FIXED: PREMIUM AUTHENTICATION UI, HWID SYSTEM, & AUTO-UPDATER LOOP BUG
-- ============================================
...
local function supremeKeyValidation()
    ...
    -- ==========================================
    -- 🛠️ [FIX] BYPASS LOGIN JIKA SUDAH AUTHENTICATED (UNTUK FITUR UPDATE)
    -- ==========================================
    ...
    if getgenv().keyExpireTime and getgenv().keyExpireTime > GetRealTime() then
        return true -- Langsung lolos tanpa munculin UI Key lagi saat Update!
    end
    -- ==========================================
    ...
    local function checkKey(rawInput)
        ...
        if not keyData.hwid or keyData.hwid == "" then
            Player:Kick("Key valid, tapi belum didaftarkan ke Akun Anda. Berikan UserID Anda (" .. playerUserID .. ") ke Admin.")
            return
        end

        if tostring(keyData.hwid) ~= playerUserID then
            Player:Kick("SECURITY ALERT: Key ini sudah terikat pada Akun lain! Beli key baru.")
            return
        end
...
-- ==========================================
-- 🛠️ [FIX] CLEANUP SEMUA UI LAMA (TERMASUK UI LOGIN)
-- ==========================================
pcall(function()
    for _, gui in pairs(CoreGui:GetChildren()) do 
        if gui.Name:find("TROXZY_VIP") or gui.Name:find("TroxzyKeyAuth") then gui:Destroy() end 
    end
...
function ConnectMapDetection()
    -- Win script mengelola deteksi sendiri
end
...
local function AddToggle(tabKey, name, stateKey) local tabIdx; for i, t in ipairs(tabItems) do if t.key==tabKey then tabIdx=i; break end end; if not tabIdx then return end; local f = Instance.new("Frame"); f.Size = UDim2.new(1,0,0,40); f.BackgroundColor3 = DARK_THEME.ToggleBg; addCorner(f,6); addStroke(f, DARK_THEME.Border, 1, 0.9); f.Parent = tabContents[tabIdx].scroll; RegisterThemeObject(f, "BackgroundColor3", DARK_THEME.ToggleBg, LIGHT_THEME.ToggleBg)
    ...
    btn.MouseButton1Click:Connect(function()
        ...
        elseif stateKey == "AutoFarm" then
            ...
            if state then StartWinEngine() else StopWinEngine() end
...
local dTitle = createDashLabel("<b>⚔️ SYSTEM OVERVIEW</b>", 1, DARK_THEME.Accent); dTitle.Font = Enum.Font.GothamBlack; dTitle.TextSize = 12
createDashDivider(2)
...
loadStats()
setupAutoReconnect()
notify("SPECTRAL BLADE v22.1 - Updater Fixed", "System")
print("TROXZY: Auto-Update Loop Bug Fixed.")
