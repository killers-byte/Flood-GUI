-- ============================================
-- TROXZY VIP v24.3 "SPECTRAL BLADE" [BACKEND CORE]
-- Badan Intelijen Negara - AUTO WIN PROTOCOL
-- 🔥 FEATURE: HYBRID TAS, AUTO REBIRTH V2, STRICT OVERVIEW & SAFE MAP LOAD
-- ============================================
...
-- ==============================================
-- 👁️ STRICT ADVANCED OVERVIEW (ADMIN & SPECTATOR)
-- ==============================================
...
local function getAdminNames()
    local names = {}
    for _, p in ipairs(getAdminPlayers()) do table.insert(names, p.Name) end
    return names
end
...
local function getSpectators()
    ...
    if not myChar or not myChar:FindFirstChild("HumanoidRootPart") then return specs end
    local myRoot = myChar.HumanoidRootPart

    for _, p in pairs(Players:GetPlayers()) do
        if p ~= Player then
            local isSpectatingMe = false
            pcall(function()
                for _, val in pairs(p:GetChildren()) do
                    local vName = val.Name:lower()
                    if vName:match("spectat") or vName:match("target") or vName:match("cam") then
                        if (val:IsA("ObjectValue") and val.Value == Player) or (val:IsA("StringValue") and val.Value == Player.Name) then
                            isSpectatingMe = true
                        end
                    end
                end
            end)
            if not isSpectatingMe and p.Character and p.Character:FindFirstChild("HumanoidRootPart") then
                local theirChar = p.Character
                local theirRoot = theirChar.HumanoidRootPart
                local distance = (myRoot.Position - theirRoot.Position).Magnitude
                if distance < 25 then
                    local hum = theirChar:FindFirstChild("Humanoid")
                    local isGhost = theirRoot.Transparency >= 0.5
                    local isDead = hum and hum.Health <= 0
                    local noCollisions = not theirRoot.CanCollide
                    if isGhost or isDead or noCollisions then isSpectatingMe = true end
                end
            end
            if isSpectatingMe then table.insert(specs, p) end
        end
    end
    return specs
end
...
local function getSpectatorNames()
    local names = {}
    for _, p in ipairs(getSpectators()) do table.insert(names, p.Name) end
    return names
end
-- ==============================================
...
local function activateGhostMode()
    if STATE.isGhostMode then return end; STATE.isGhostMode = true; getgenv().TomatoAutoFarm = false; STATE.CurrentlyFarming = false; 
...
local function deactivateGhostMode() if not STATE.isGhostMode then return end; STATE.isGhostMode = false; if getgenv().TroxzyAPI and getgenv().TroxzyAPI.UIHooks.show then getgenv().TroxzyAPI.UIHooks.show() end end
...
local function handleAdminDetection()
    ...
    if count > 0 then
        if count > lastAdminCount and (os.clock() - lastAdminAlert >= 10) then
            ...
            if getgenv().TomatoAutoFarm or STATE.AUTO_QUEUE_ENABLED or STATE.TAS_RUNNING then activateGhostMode() end
        end
    else if STATE.isGhostMode then deactivateGhostMode() end end
...
local function ExecuteTAS()
    ...
    TAS_COROUTINE = coroutine.create(function()
        STATE.TAS_RUNNING = true
        pcall(func)
        STATE.TAS_RUNNING = false
        TAS_COROUTINE = nil
        if STATE.AUTO_QUEUE_ENABLED then STATE.mapCompleted = true end
        if getgenv().TroxzyAPI and getgenv().TroxzyAPI.UIHooks.updateTASStatus then getgenv().TroxzyAPI.UIHooks.updateTASStatus("  Status: READY") end
    end)
...
local function StartWinEngine()
    ...
    WinFarmCoroutine = coroutine.create(function() WinFarmRunning = true; STATE.CurrentlyFarming = true; pcall(func); WinFarmRunning = false; STATE.CurrentlyFarming = false; WinFarmCoroutine = nil end)
...
-- ==============================================
-- 🔥 HYBRID AUTO-WIN (TAS + AUTO FARM FALLBACK)
-- ==============================================
local SUPPORTED_TAS_MAPS = {
    "Abandoned Cemetery", "Abandoned Facility", "Abandoned Junkyard", "Active Volcanic Mines",
    "Antiquated Railways", "Aquatic Reservoir", "Archipelago", "Arctic Grotto", "Axiom",
    "Bathroom Leak", "Beep Block Station", "Beneath The Ruins", "Blue Moon", "Buried Oasis",
    "Cadaver Creek", "Calamity Kingdom", "Castle Tides", "Cave System", "Central Mass Array",
    "Chaoz Japan", "Classic Canyon", "Closing Hours", "Club Quarry", "Construction Thrill",
    "Crystal Caverns", "Cyberpunk District", "Dark Sci-Forest", "Decaying Silo", "Decrepit Seas",
    "Desolate Domain", "Despotic Ruins", "Distorted Ignis Dimension", "Dormant Vale",
    "Dreamscape Skylines", "Eerie Peaks", "Factory Center", "Fall Equinox", "Fallen (Classic)",
    "Fallen", "Familiar Ruins", "Flood Island", "Flooded Studio", "Footlight Lane",
    "Forgotten Tombs", "Forsaken Era", "Frostbite Coastline", "Gloomy Manor", "Golden Passage",
    "Graveyard Cliffside", "Grumbling Mineshafts", "Hovering Enclaves", "Icy Spires", "Ignis Peaks",
    "Infiltration", "Lava Tower", "Lost City of Gold", "Lost Desert", "Lost Woods", "Luminance",
    "Magmatic Mines", "Marigold Meadows", "Marred Dreams", "Mysterium", "Mystic Fortress",
    "Nimble Valley", "Northern Mill", "Oriental Grove", "Outlier of a Coppice Carcass",
    "Pascha Equinox", "Pitfall Temple", "Poisonous Chasm", "Poisonous Forest", "Poisonous Valley",
    "Relic Valley", "Retro Coast", "Rustic Jungle", "Sakura Falls", "Sandstorm Dunes",
    "Sandswept Ruins", "Sapphire Falls", "Satomi Springs", "Sedimentary Temple", "Shimmering Delta",
    "Sinking Ship", "Sky Sanctuary", "Snowscape Odyssey", "Snowy Peaks", "Snowy Stronghold",
    "Spider Dungeon", "Star Manor", "Starry Fields", "Submerging Coastland", "Sulphureous Sea",
    "Summit", "Sunken Citadel", "Sunlit Villas", "Toxic Woods", "Toybox Trouble",
    "Whirlwind Wasteland", "Whispering Nocturnal", "Wild Savannah", "Wildwood Waterways",
    "Windswept Valley", "Wintertide Haven", "Zemblanity"
}

local function isMapInTAS(mapName)
    for _, map in ipairs(SUPPORTED_TAS_MAPS) do
        if string.lower(map) == string.lower(mapName) then return true end
    end
    return false
end

-- ==============================================
-- FIX BUG STUCK LOADING: Anti-Spam Loading Debounce
-- ==============================================
local MapLoadDebounce = false

function StartAutoQueue()
    if AutoQueueListener then AutoQueueListener:Disconnect() end
    STATE.moveToLift = false
    STATE.mapCompleted = false
    MapLoadDebounce = false
    ...
    AutoQueueListener = Multiplayer.ChildAdded:Connect(function(newMap)
        ...
        -- FIX 1: FILTER KETAT. Abaikan jika yang di-load bukan Folder atau Model (mencegah part sampah me-trigger loading)
        if not newMap:IsA("Folder") and not newMap:IsA("Model") then return end
        
        -- FIX 2: DEBOUNCE. Mencegah script mengeksekusi multiple TAS saat sistem membaca banyak part sekaligus.
        if MapLoadDebounce then return end
        MapLoadDebounce = true
        
        task.wait(1) -- Beri waktu singkat untuk game menyusun map

        local mapName = "Unknown"
        pcall(function() 
            local s = newMap:FindFirstChild("Settings") or newMap:WaitForChild("Settings", 3)
            if s then mapName = s:GetAttribute("MapName") or newMap.Name end 
        end)
        Stats.currentMap = mapName

        -- FIX 3: TIMEOUT SYSTEM. Tidak akan nunggu selamanya jika gagal teleport.
        local timeoutCounter = 0
        repeat 
            task.wait(0.5)
            timeoutCounter = timeoutCounter + 0.5
        until Check("InGame") or not STATE.AUTO_QUEUE_ENABLED or timeoutCounter > 15

        -- Jika gagal masuk setelah 15 detik, batalkan supaya tidak stuck selamanya
        if timeoutCounter > 15 or not STATE.AUTO_QUEUE_ENABLED then 
            MapLoadDebounce = false
            return 
        end
        
        -- FIX 4: SAFE BUFFER. Beri waktu transisi agar fisika karakter tidak bentrok dengan map baru (Penyebab utama stuck loading).
        task.wait(2)
        
        STATE.mapCompleted = false

        if isMapInTAS(mapName) then
            notify("Memulai TAS Engine - " .. mapName, "Hybrid System")
            if WinFarmRunning then StopWinEngine() end
            getgenv().TomatoAutoFarm = false
            task.spawn(ExecuteTAS)
        else
            notify("TAS Tidak Tersedia - Auto Farm Aktif", "Hybrid System")
            if STATE.TAS_RUNNING and TAS_COROUTINE then 
        ...
        -- Buka kuncian debounce hanya jika sudah selesai/keluar dari map
        task.spawn(function()
            repeat task.wait(1) until not Check("InGame")
            MapLoadDebounce = false
        end)
    end)
...
function StopAutoQueue() 
    ...
    STATE.moveToLift = false
    STATE.AUTO_QUEUE_ENABLED = false
    if STATE.TAS_RUNNING and TAS_COROUTINE then pcall(coroutine.close, TAS_COROUTINE); STATE.TAS_RUNNING = false end 
end

task.spawn(function()
    while task.wait(0.5) do
        if (STATE.AUTO_QUEUE_ENABLED or getgenv().TomatoAutoFarm) and not STATE.panicActive and not STATE.isGhostMode then
            local char = Player.Character
            if char and char:FindFirstChild("HumanoidRootPart") and not Check("InGame") and not Check("InLift") and not STATE.TAS_RUNNING and not STATE.CurrentlyFarming then STATE.moveToLift = true; liftTarget = findLiftPosition() else STATE.moveToLift = false end
        else STATE.moveToLift = false end
    end
end)

TrackConnection(RunService.Heartbeat:Connect(function()
    if STATE.moveToLift and (STATE.AUTO_QUEUE_ENABLED or getgenv().TomatoAutoFarm) and not STATE.panicActive and not STATE.TAS_RUNNING then
        ...
        if hum and hrp and hum.Health > 0 and not Check("InGame") and not Check("InLift") and liftTarget then
            if (liftTarget - hrp.Position).Magnitude > 3 then hum:MoveTo(liftTarget); hum.WalkSpeed = 25 else pcall(function() AddedWaiting:FireServer() end); STATE.moveToLift = false end
        end
    end
end))

TrackConnection(Player.CharacterAdded:Connect(function() refreshNoclip(); ncActive = false; if not Check("InGame") then STATE.mapCompleted = false; MapLoadDebounce = false end end))

-- =============================================
-- 🔥 AUTO REBIRTH SYSTEM v2.0
-- =============================================
local REBIRTH_CONFIG = {
    ENABLED = true, CHECK_INTERVAL = 3, AUTO_RESUME = true, MAX_RETRIES = 5, REBIRTH_DELAY_MIN = 2, REBIRTH_DELAY_MAX = 5,
    MAX_LEVEL = 100, LEADERSTAT_NAME = "Level", REMOTE_NAMES = { "rebirth", "reborn", "prestige", "ascend", "reset", "newlife", "dorebirth" },
    BUTTON_NAMES = { "rebirth", "reborn", "prestige", "ascend", "reincarnate" }, LEVEL_ATTRIBUTE = "Level", REBIRTH_STAT_NAME = "Rebirth"
}
        ...
        while REBIRTH_CONFIG.ENABLED do
            ...
            if not STATE.panicActive and not RebirthState.isRebirthing then
                ...
                if currentLevel >= REBIRTH_CONFIG.MAX_LEVEL and currentLevel > 0 then
                    ...
                    local wasFarming, wasTAS, wasQueue = getgenv().TomatoAutoFarm, STATE.TAS_RUNNING, STATE.AUTO_QUEUE_ENABLED
                    if wasFarming then StopWinEngine() end
                    if wasTAS and TAS_COROUTINE then pcall(coroutine.close, TAS_COROUTINE); TAS_COROUTINE = nil; STATE.TAS_RUNNING = false end
                    task.wait(0.5); local rebirthSuccess = executeRebirth()
                    if rebirthSuccess and REBIRTH_CONFIG.AUTO_RESUME then task.wait(2); if wasQueue then StartAutoQueue() elseif wasFarming then getgenv().TomatoAutoFarm = true; StartWinEngine() elseif wasTAS then task.spawn(ExecuteTAS) end end
                end
            end
        end
    end)
end
local function StopAutoRebirth() REBIRTH_CONFIG.ENABLED = false; AutoRebirthLoop = nil end
...
local API = {
    ...
    activatePanicMode = activatePanicMode, deactivatePanicMode = deactivatePanicMode, applyFloodColors = applyFloodColors, ExecuteTAS = ExecuteTAS, StartAutoQueue = StartAutoQueue, StopAutoQueue = StopAutoQueue,
    StartWinEngine = StartWinEngine, StopWinEngine = StopWinEngine, forceReconnect = forceReconnect, notify = notify, Rebirth = getgenv().TroxzyAPI_Rebirth, UIHooks = {}
}
...
if success and uiScript then local runUI, err = loadstring(uiScript); if runUI then runUI() else warn("Gagal membaca kode UI dari GitHub: " .. tostring(err)) end else warn("Gagal terhubung ke GitHub RAW UI!") end
...
TrackConnection(RunService.Heartbeat:Connect(function()
    ...
    local adminsStr = table.concat(getAdminNames(), ", ")
    STATE.CurrentAdmins = adminsStr ~= "" and adminsStr or "None"
    
    local specsStr = table.concat(getSpectatorNames(), ", ")
    STATE.CurrentSpectators = specsStr ~= "" and specsStr or "None"

    pcall(function()
        local ch = Player.Character; if not ch then return end; local hum = ch:FindFirstChild("Humanoid"); if not hum then return end
        if CONFIG.NOCLIP and not STATE.CurrentlyFarming then refreshNoclip(); applyNoclip(true) elseif not STATE.CurrentlyFarming then applyNoclip(false) end
...
notify("TROXZY VIP - SAFE LOAD & ANTI-STUCK FIXED", "System")
