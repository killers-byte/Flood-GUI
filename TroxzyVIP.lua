...
-- Badan Intelijen Negara - STRICT ADMIN DETECT + HYBRID FIX + STEALTH OPS + AUTO REBIRTH
-- ============================================
...
local function supremeKeyValidation()
    ...
    local function parseExpiry(expiry)
        if expiry == "permanent" or expiry == "owner" then return 9999999999 end
...
local CONFIG = {
    ...
    AUTO_REBIRTH = false -- FITUR AUTO REBIRTH DITAMBAHKAN
}
...
-- ==========================================
-- ?? SYSTEM HYBRID: AUTO REBIRTH EKSEKUTOR ??
-- ==========================================
        ...
        if gui:IsA("TextButton") or gui:IsA("ImageButton") then
            ...
            if text:find("rebirth") or text:find("kelahiran") then
                if gui.Visible then
                    ...
                    task.wait(0.5) -- Tunggu popup konfirmasi muncul
                    
                    -- Cari dan eksekusi tombol Konfirmasi
...
-- Loop khusus Auto Rebirth biar jalan mulus di Hybrid
task.spawn(function()
