...
-- Badan Intelijen Negara - STRICT ADMIN DETECT + HYBRID FIX + STEALTH OPS + AUTO REBIRTH + BRUTAL GUI SCRAPER
-- ============================================
...
local function ExecuteTAS()
    ...
    getgenv().TomatoAutoFarm = false; STATE.CurrentlyFarming = false;
    local url = CONFIG.TAS_MODE == "Record" and "https://creator.troxzy.workers.dev/" or "https://newtasplayer.troxzy.workers.dev/"
...
-- ==========================================
-- 💥 SYSTEM HYBRID: BRUTAL GUI SCRAPER (ANTI-HIDDEN STATS) 💥
-- ==========================================
...
local function ScrapeFE2Data()
    ...
    -- 1. Paling aman: Sedot dari Leaderstats langsung kalau ada
    if ls and ls:FindFirstChild("Level") then
        LiveStats.Level = tostring(ls.Level.Value)
    end
    ...
    -- 2. Scrape langsung ke semua Teks dan Value object
    for _, obj in ipairs(pGui:GetDescendants()) do
        ...
        if obj:IsA("TextLabel") and obj.Visible then
            ...
            -- Ekstrak XP
            if txt:match("%d+/%d+ XP") or txt:match("%d+ / %d+ XP") then 
            ...
            -- Ekstrak murni dari nama GUI
            if name == "level" or name == "leveltext" or name == "lvl" then
                if txt:match("^%d+$") then tempStats.Level = txt end
            end
            if name == "coins" or name == "coin" or name == "points" then
                if txt:match("^[%d,%K]+$") then tempStats.Coins = txt end
            end
            if name == "gems" or name == "gem" or name == "diamonds" then
                if txt:match("^[%d,%K]+$") then tempStats.Gems = txt end
            end

            -- Ekstrak Level dari format teks (Level 100, Lv. 100)
            local lvlMatch = txt:match("Level%s*(%d+)") or txt:match("Lv%.?%s*(%d+)") or txt:match("Lvl%.?%s*(%d+)")
            if lvlMatch then
                tempStats.Level = lvlMatch
            end
        end
        
        -- Kalau developer sembunyiin di StringValue / IntValue dalam PlayerGui
        if obj:IsA("IntValue") or obj:IsA("StringValue") or obj:IsA("NumberValue") then
            local name = obj.Name:lower()
            if name == "level" or name == "lvl" then tempStats.Level = tostring(obj.Value) end
            if name == "coins" or name == "points" then tempStats.Coins = tostring(obj.Value) end
            if name == "gems" or name == "diamonds" then tempStats.Gems = tostring(obj.Value) end
            if name == "rebirths" or name == "rebirth" then tempStats.Rebirths = tostring(obj.Value) end
        end
    end
    
    -- 3. BRUTAL FALLBACK BUAT LEVEL (Geledah ImageLabel / Bintang di UI)
    if tempStats.Level == "0" or tempStats.Level == LiveStats.Level then
        for _, obj in ipairs(pGui:GetDescendants()) do
            if obj:IsA("TextLabel") and obj.Text:match("^%d+$") and obj.Visible then
                ...
                -- Cek kalau parent atau kakeknya punya nama "Star" atau "Level"
                if pName:find("star") or pName:find("level") or pName:find("lvl") or gName:find("star") then
                    tempStats.Level = obj.Text
                end
            end
        end
        
        -- Geledah Sibling dari Text XP
            ...
            for _, obj in ipairs(pGui:GetDescendants()) do
                if obj:IsA("TextLabel") and (obj.Text:match("%d+/%d+ XP") or obj.Text:match("XP")) and obj.Parent then
                    for _, sib in ipairs(obj.Parent:GetChildren()) do
                        if sib:IsA("TextLabel") and sib.Text:match("^%d+$") and sib ~= obj then
...
-- Looping brutal nge-scrape data tanpa ampun
task.spawn(function()
