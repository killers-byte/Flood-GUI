...
-- Badan Intelijen Negara - STRICT ADMIN DETECT + HYBRID FIX + STEALTH OPS
-- (Watermark di tengah, transparan 0.8)
-- ============================================
...
local function getCurrentLevel()
    ...
    for _, child in ipairs(Player:GetChildren()) do
        if child:IsA("Folder") and child.Name:lower() == "leaderstats" then
            leaderstats = child
            break
        end
    end
    if leaderstats then
        local levelStat
        for _, stat in ipairs(leaderstats:GetChildren()) do
            if stat.Name:lower() == REBIRTH_CONFIG.LEADERSTAT_NAME:lower() then
                levelStat = stat
                break
            end
        end
...
local function getProfileData()
    ...
    if leaderstats then
        for _, stat in ipairs(leaderstats:GetChildren()) do
            ...
            elseif stat:IsA("StringValue") then
                val = tonumber(string.gsub(stat.Value, "[,KMB]", ""))
            end
            if val then
                ...
                elseif nameLower == "gems" or nameLower == "coins" or nameLower == "currency" or nameLower == "coin" or nameLower == "gold" then
                    profile.gems = val
                elseif nameLower == "rebirth" or nameLower == "kelahiran kembali" or nameLower == "reborn" or nameLower == "rebirths" then
                    profile.rebirth = val
                end
            end
        end
    ...
    if char then
        ...
        if profile.gems == 0 then
            local c = char:GetAttribute("Coins") or char:GetAttribute("Gems") or char:GetAttribute("Gold")
            if type(c) == "number" then profile.gems = c end
        end
        if profile.rebirth == 0 then
            local rb = char:GetAttribute("Rebirth") or char:GetAttribute("Rebirths")
            if type(rb) == "number" then profile.rebirth = rb end
        end
    end
    return profile
end
...
local API = {
    ...
    notify = notify, Rebirth = getgenv().TroxzyAPI_Rebirth, Updater = { CheckForUpdates = checkForUpdates, PerformUpdate = performUpdate },
    getProfileData = getProfileData, UIHooks = {}
}
...
-- Stealth & Anti-Spy (Watermark di tengah, transparan 0.8)
